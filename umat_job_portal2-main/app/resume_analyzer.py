...
class ResumeAnalyzer:
    ...
    def extract_skills(self, text):
        """Extract and categorize skills from resume text using enhanced detection."""
        if not text:
            return {}
        ...
        extracted_skills = self._extract_skills_comprehensive(text, text_lower)
    ...
    def _extract_skills_comprehensive(self, text, text_lower):
        """Comprehensive skill extraction using multiple detection methods."""
        skills = defaultdict(list)
        
        # Method 1: Basic taxonomy matching
        self._extract_skills_taxonomy(text_lower, skills)
        
        # Method 2: Case-sensitive matching
        self._extract_skills_case_sensitive(text, skills)
        
        # Method 3: Pattern-based matching
        self._extract_skills_patterns(text, skills)
        
        # Method 4: spaCy NER extraction
        self._extract_skills_spacy(text, skills)
        
        # Method 5: Context-based extraction
        self._extract_skills_context(text_lower, skills)
        
        return skills
    
    def _extract_skills_taxonomy(self, text_lower, skills):
    ...
    def _extract_skills_case_sensitive(self, text, skills):
    ...
    def _extract_skills_patterns(self, text, skills):
    ...
    def _extract_skills_spacy(self, text, skills):
        """Extract skills using spaCy NER and additional processing."""
        if not self.nlp:
            return
        ...
        for ent in doc.ents:
            if ent.label_ in ["PRODUCT", "ORG", "GPE"]:
                ...
                tech_indicators = [
                    ...
                    "database", "server", "api", "sdk", "ide", "editor", "compiler",
                    "interpreter", "runtime", "engine", "suite", "studio", "workspace"
                ]
                
                if any(indicator in ent_text.lower() for indicator in tech_indicators):
                    skills["other_technologies"].append(ent_text)
        
        # Extract noun chunks that might be skills
        for chunk in doc.noun_chunks:
            chunk_text = chunk.text.strip().lower()
            # Check if noun chunk contains known skill keywords
            skill_keywords = [
                "development", "programming", "framework", "tool", "technology", "language",
                "database", "server", "api", "sdk", "library", "platform", "system"
            ]
    ...
    def _extract_skills_context(self, text_lower, skills):
        ...
        for pattern, context_type in context_patterns:
            ...
            for match in matches:
                ...
                self._extract_skills_from_context(context_text, skills)
    
    def _extract_skills_from_context(self, context_text, skills):
    ...
    def extract_experience(self, text):
        """Extract and analyze professional experience details using enhanced analyzer."""
        try:
            # Use the enhanced work experience analyzer
            from .work_experience_analyzer import extract_work_experience
            result = extract_work_experience(text)
            
            # Convert to the expected format for the template
            experience = {
                "years": result.get("total_years", 0),
                "level": result.get("experience_level", "entry"),
                "positions": [exp.get('position', 'Unknown') for exp in result.get('work_experiences', [])],
                "domains": [exp.get('company', 'Unknown') for exp in result.get('work_experiences', [])],
                "work_experiences": result.get('work_experiences', []),
                "total_months": result.get('total_months', 0),
                "earliest_start_date": result.get('earliest_start_date'),
                "latest_end_date": result.get('latest_end_date'),
            ...
            logger.error(f"Error in enhanced experience extraction: {e}")
