...
from app.models import User, UserRole, Candidate, Resume, Job, JobMatch, Application, Interview, ApplicationStatus, Event, InterviewLevel, InterviewLevelStatus
from app.forms import RegistrationForm, LoginForm, ResumeUploadForm, JobForm, ApplicationForm, InterviewForm, RescheduleInterviewForm, ProfileEditForm, EventForm
...
@bp.route("/applications/<int:application_id>/interviews/<int:interview_id>/manage", methods=["GET", "POST"])
@login_required
@admin_required
def manage_interview(application_id, interview_id):
    """Manage and reschedule interviews"""
    application = Application.query.get_or_404(application_id)
    interview = Interview.query.get_or_404(interview_id)
    
    # Ensure the interview belongs to the application
    if interview.application_id != application_id:
        flash("Interview not found for this application.", "danger")
        return redirect(url_for("main.view_application", application_id=application_id))
    
    form = RescheduleInterviewForm()
    
    if request.method == "GET":
        # Populate form with current interview data
        form.interview_date.data = interview.scheduled_date.date()
        form.interview_time.data = interview.scheduled_date.strftime('%H:%M')
        form.interview_level.data = interview.interview_level.value
        form.interview_type.data = interview.interview_type
        form.location_or_link.data = interview.location_or_link
        form.interviewer.data = interview.interviewer_id
        form.level_status.data = interview.level_status.value
        form.notes.data = interview.notes
        form.interviewer_notes.data = interview.interviewer_notes
        
        if interview.next_level_date:
            form.next_level_date.data = interview.next_level_date.date()
            form.next_level_time.data = interview.next_level_date.strftime('%H:%M')
    ...
    if form.validate_on_submit():
        try:
            ...
            interview.interview_level = form.interview_level.data
            interview.interview_type = form.interview_type.data
            interview.location_or_link = form.location_or_link.data
            interview.interviewer_id = form.interviewer.data
            interview.level_status = form.level_status.data
            interview.notes = form.notes.data
            interview.interviewer_notes = form.interviewer_notes.data
            
            # Handle next level scheduling
            if form.schedule_next_level.data and form.next_level_date.data and form.next_level_time.data:
                next_date_str = form.next_level_date.data.strftime('%Y-%m-%d')
                next_time_str = form.next_level_time.data
                next_level_datetime = datetime.strptime(f"{next_date_str} {next_time_str}", '%Y-%m-%d %H:%M')
                interview.next_level_date = next_level_datetime
                
                # Create next level interview if it doesn't exist
                next_level = interview.get_next_level()
                if next_level:
                    existing_next = Interview.query.filter_by(
                        application_id=application_id,
                        interview_level=next_level
                    ).first()
                    
                    if not existing_next:
                        next_interview = Interview(
                            application_id=application_id,
                            scheduled_date=next_level_datetime,
                            interview_type=interview.interview_type,
                            location_or_link=interview.location_or_link,
                            interview_level=next_level,
                            level_status=InterviewLevelStatus.pending,
                            interviewer_id=form.interviewer.data,
                            notes=f"Next level interview scheduled after {interview.interview_level.value} level"
                        )
                        db.session.add(next_interview)
            else:
                interview.next_level_date = None
            
            # Update application status based on interview level status
            if form.level_status.data == 'passed':
                if interview.interview_level == InterviewLevel.university:
                    application.status = ApplicationStatus.offer_pending
                elif interview.interview_level in [InterviewLevel.department, InterviewLevel.faculty]:
                    application.status = ApplicationStatus.interview_scheduled
            elif form.level_status.data == 'failed':
            ...
            try:
                ...
                send_email(
                    ...
                    template="email/interview_updated.html",
                    user=candidate,
                    job=job,
                    interview=interview,
                    application=application
                ...
                if admin_emails:
                    send_email(
                        subject=f"Interview Updated - {job.title}",
                        recipients=admin_emails,
                        template="email/interview_updated_admin.html",
                        candidate=candidate,
                        job=job,
                        interview=interview,
                        application=application
                ...
                logger.error(f"Failed to send interview update notification emails: {email_err}", exc_info=True)
            ...
            flash("Error updating the interview. Please try again.", "danger")
...
@bp.route("/applications/<int:application_id>/interviews/<int:interview_id>/cancel", methods=["POST"])
@login_required
@admin_required
def cancel_interview(application_id, interview_id):
    """Cancel an interview"""
    application = Application.query.get_or_404(application_id)
    interview = Interview.query.get_or_404(interview_id)
    ...
    try:
        interview.level_status = InterviewLevelStatus.failed
        interview.notes = f"Cancelled on {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}"
        ...
        try:
            ...
            send_email(
                ...
                recipients=[candidate.email],
                template="email/interview_cancelled.html",
                user=candidate,
                job=job,
                interview=interview,
                application=application
            ...
            logger.error(f"Failed to send interview cancellation email: {email_err}", exc_info=True)
        ...
        flash("Error cancelling the interview. Please try again.", "danger")
...
def my_interviews():
    ...
    for app in applications:
        # Get the first interview for this application
        interview = app.interviews.first() if app.interviews else None
        if interview and interview.scheduled_date > now:
    ...
    if form.validate_on_submit():
        try:
            ...
            if form.test_email.data and form.test_recipient.data:
                # Ensure only one of TLS/SSL is enabled
                if form.mail_use_ssl.data:
                    form.mail_use_tls.data = False
                elif form.mail_use_tls.data:
                    form.mail_use_ssl.data = False
                ...
                if test_result:
                    ...
                    last_err = current_app.config.get('LAST_MAIL_ERROR', 'Unknown error')
                    flash(f"Failed to send test email: {last_err}", "danger")
            ...
            # If this is a save settings request
                ...
                if os.path.exists(env_file):
                    with open(env_file, 'r') as f:
                        for line in f:
                            ...
                            if not line.strip().startswith(('MAIL_SERVER=', 'MAIL_PORT=', 'MAIL_USE_TLS=', 'MAIL_USE_SSL=', 
                ...
                env_content.append(f"MAIL_USE_SSL={'true' if form.mail_use_ssl.data else 'false'}")
